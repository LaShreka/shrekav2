...
const PremiumKey = 'c1d162b46e634f389efa1ac715f03d03'
...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				...
				only: {
					...
					userB: `──「 LISTA 」──\nOla mano👋!\nPercebi que você não está registrado como amigo do meu dono 𓆩ꬸꭼꮮ_ꮪꮋꭱꭼꮶꬸ𓆪 no dados✋😞\n\nPor favor,use o comando : ${prefix}regist nome|idade\nExemplo : ${prefix}regist SHREK|17\n\n──「 𓆩ꬸꭼꮮ_ꮪꮋꭱꭼꮶꬸ𓆪 」──`,
		...
		if (getmemex <= memberLimitss) {
		setTimeout( () => {
		client.groupLeave(from) 
		}, 2000)
		setTimeout ( () => {
		client.sendMessage(from, `Desculpe,Tchau👋`, text)
		}, 1000)
		setTimeout( () => {
		client.sendMessage(from, `Desculpe ${name} Não é possível entrar no grupo por causa da quantidade de membros do grupo ${groupMetadata.subject} Limite de Membro Não Correspondido\n\nMembro Mínimo ${memberLimitss}`, text)
		}, 0)
	}
...
client.sendMessage('./lindy/websexohjmulher.mp3', {quoted: mek, ptt:true})
            ...
            break
				...
				try {
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: `NOME : *${groupName}*\nMODE SIMI: ${Simihh}\nMODE WELCOME: ${Welcomee}\nMODE ANTIRACISMO: ${AntiRaccism}\nMODE ANTILINK: ${AntiLinkk}\nMODE ANTITRAVA: ${AntiVirtexx}\nMODE ANIME: ${ModeAnime}\nMODE NSFW: ${Nsfww}\nMEMBER : *${groupMembers.length}*\nADMIN : *${groupAdmins.length}*\nDESK : ${groupDesc}`})
					break
		...
		case 'verify':
							client.updatePresence(from, Presence.composing)
					if (isUser) return reply('Você já tá registrado!!!')
					if (isBanned) return reply(mess.only.benned)
					user.push(sender)
...
case 'pinterest1':
  if (isBanned) return reply(mess.only.benned) 
				...
				if (!isUser) return reply(mess.only.userB)
					...
					if (args.length < 1) return reply('Cadê o texto, hum')
					reply(mess.wait)
					anu = await fetchJson(`https://api.zeks.xyz/api/snowwrite?text1=${gbl7w}&text2=${gbl8w}&apikey=xptnbot352`, {method: 'get'})
...
case 'ffmaker':
				...
				if (!isUser) return reply(mess.only.userB)
					...
					if (args.length < 1) return reply('Cadê o texto, hum')
					reply(mess.wait)
					anu = await fetchJson(`https://api.vhtear.com/logoff?hero=${gbl7w}&text=${gbl8w}&apikey=${VthearApi}`, {method: 'get'})
					buffertfd = await getBuffer(anu.result)
					client.sendMessage(from, buffertfd, image, {quoted: mek})
					break
					case 'ban':
...
ranp = getRandom('.gif')
                ...
                axios.get('https://nekos.life/api/v2/img/boobs').then(res => {
                	client.sendMessage(from, res.data.url, 'bakaaa hentaii>~<');
                });
                break
				case 'nsfwtrap':
				    try {
...
case 'afk':
				...
				if (!isUser) return reply(mess.only.userB)
                                        ...
                                        client.sendMessage(from, afk, text, {quoted: mek})
                                        break
case 'tiktok1':
				if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
					...
					if (!isUrl(args[0]) && !args[0].includes('tiktok.com')) return reply(mess.error.Iv)
					reply(mess.wait)
					anu = await fetchJson(`https://api.vhtear.com/tiktokdl?link=${url}&apikey=${VthearApi}`, {method: 'get'})
					if (anu.error) return reply(anu.error)
					buffer = await getBuffer(anu.result)
					client.sendMessage(from, buffer, video, {quoted: mek})
					break
case 'tiktok2':
				...
				if (!isUser) return reply(mess.only.userB)
					...
					if (!isUrl(args[0]) && !args[0].includes('tiktok.com')) return reply(mess.error.Iv)
					reply(mess.wait)
					anu = await fetchJson(`https://api.zeks.xyz/api/tiktok?url=&apikey=xptnbot352`, {method: 'get'})
					if (anu.error) return reply(anu.error)
					buffer = await getBuffer(anu.result)
					client.sendMessage(from, buffer, video, {quoted: mek})
					break
				...
				if (!isUser) return reply(mess.only.userB)
                    ...
                    buffer5 = await getBuffer(`https://api.zeks.xyz/api/text3dbox?apikey=xptnbot352&text=${teks}`, {method: 'get'})
			   ...
			   if (!isPremium) return reply(mess.only.premi)
					 ...
					 love = `${body.slice(7)}`
					 buffer = await getBuffer(`https://api.vhtear.com/lovemessagetext?text=${love}&apikey=${VthearApi}`, {method: 'get'})
			...
			if (isBanned) return reply(mess.only.benned)    
				...
				if (!isUser) return reply(mess.only.userB)
					...
					if (!isOwner) return reply('Comando registro só pro dono')
					anu = await client.chats.all()
					client.setMaxListeners(25)
					for (let _ of anu) {
						client.deleteChat(_.jid)
					}
					reply('🗑️Excluindo tudo, até seus nuds senhor...🗑️')
					break
...
case 'antilink':
				if (isBanned) return reply(mess.only.benned)  
					...
					if (args.length < 1) return reply('Hmmmm')
					if (Number(args[0]) === 1) {
						if (isAntiLink) return reply('Já esta ativo.')
						antilink.push(from)
						fs.writeFileSync('./src/antilink.json', JSON.stringify(antilink))
						reply('Ativou com sucesso o antilink no grupo 😉️')
					} else if (Number(args[0]) === 0) {
						antilink.splice(from, 1)
						fs.writeFileSync('./src/antilink.json', JSON.stringify(antilink))
						reply('Desativou com sucesso o antilink no grupo 😡️')
					} else {
						reply('1 para ativar, 0 para desativar, lerdão vc em KAKKKK')
						}
						break
case 'antivirtex':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (args.length < 1) return reply('digite para ativar on')
					if ((args[0]) === '1') {
						if (isAntiVirtex) return reply('grupo anti-virtex está ativo')
						_antivirtex.push(from)
						fs.writeFileSync('./database/json/antivirtex.json', JSON.stringify(_antivirtex))
						reply(`\`\`\`Ativado com sucesso o modo antivirtex no grupo\`\`\` *${groupMetadata.subject}*`)
					} else if ((args[0]) === '0') {
						if (!isAntiVirtex) return reply('O modo antivirtex foi desativado')
						_antivirtex.splice(from, 1)
						fs.writeFileSync('./database/json/antivirtex.json', JSON.stringify(_antivirtex))
						reply(`\`\`\`✓Modo antivirtex desativado com sucesso no grupo\`\`\` *${groupMetadata.subject}*`)
					} else {
...
case 'blackpink':
                ...
                case 'christmas':
                    ...
                    txt = args.join(" ")
                    ct = await getBuffer(`http://api.lolhuman.xyz/api/textprome/${command}?apikey=RiuApikey&text=${ct}`)
